...
def exit():
    print(f'\n{Fore.YELLOW}Press Esc to exit or Enter to continue{Style.RESET_ALL}')
    while True:
        try:
...
class FacebookKPN():
    def Download_Video(URL):
        ...
        ydl = {
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best',
            'outtmpl': 'video/%(extractor)s/%(title)s [%(resolution)s] .%(ext)s'}
...
while True:
    site = input(f'\nWhere do you want to download from {Fore.RED}(Y)outube{Style.RESET_ALL} or {Fore.BLUE}(F)acebook{Style.RESET_ALL} or {Fore.CYAN}(T)witter{Style.RESET_ALL}? ').upper()
    ...
    elif site == 'T' or site == 'TWITTER' or site == 'X':
            URL = input('\nPlease enter the video URL: ')
            TwitterKPN.Download_Video(URL)
